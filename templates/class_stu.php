...
<head>
    ...
    if (isset($_POST['join'])) {
        $cid = $_POST['cid'];
        $sql = "select * from course where c_id= '$cid' limit 1 ";
        $resl = mysqli_query($con, $sql);
        if (mysqli_num_rows($resl) >= 1) {
            $sql = "insert into enroll(u_id,c_id) values($uid, $cid)";
            mysqli_query($con, $sql);
            echo "<script>location.href = 'class_stu.php';</script>";
        }
    }
    $sql = "select *from user natural join(select *from course natural join (select c_id from enroll where u_id=$uid)temp)temp1";
    $resl = mysqli_query($con, $sql);
    $n = 0;
...
<body>
        ...
        <div class="container">
            <nav id="main-nav" class="flex items-center justify-between">
                <div class="left flex  items-center">
                    <div class="branding">
                        <a href="user_dashboard.php"><img style="width:60px;height:60px;object-fit:cover;border-radius:50%;border:3px solid var(--pure);" src="<?php if(isset($dpp)) echo $dpp; ?>" alt=""></a>
                    </div>
                    <div>
                        <a href="update_stu.php" style="font-size:15px;">edit profile</a>
                        <a href="class_stu.php" style="font-size:15px;">class room</a>
                        <div class="dropdown">
                        ...
                        <button class="btn-dorkar" onclick=openForm()>JOIN CLASS</button>
                    </div>
                </div>
    ...
    <div class="form-popup" id="myForm">
        <form action="" class="form-container" method="post" enctype="multipart/form-data">
            ...
            <input type="text" placeholder="Enter your course ID" name="cid" required>
    ...
    <div class="bodyy" id="mybody">
        <div class="containerr">
            <div class="card-wrapper">
                <?php for ($i = 0; $i < $n; $i++) { ?>
                    <a href="classlayoutfinal_stu.php?c_id=<?php echo $cc[$i];?>">
                        <div class="card">
