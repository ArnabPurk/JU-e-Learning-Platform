...
$query11 = "select *from exam where e_id='{$_GET['e_id']}'";
...
$query11 = "select * from takes_part where e_id='$eid' and u_id='$uid'";
$result11 = mysqli_query($con, $query11);
if ($result11 && mysqli_num_rows($result11) > 0) {
...
if (isset($_POST['upload'])) {
    ...
    $newfilename = $cid . $eid . $edate . '_' . $filee;
    if (!empty($filee)) {
        $folder = "exam_materials/student/" . $newfilename;
    ...
    $ftime = date('H:i');
    $fdate = date('y-m-d');
    $sql2 = "insert into takes_part(u_id,e_id,ftime,fdate,file) values('$uid','$eid','$ftime','$fdate','" . $folder . "')";
...
<head>
    ...
    <link rel="stylesheet" href="css/hudaiplus.css">
</head>
...
<body>
    ...
    <ul class="nav flex-column bg-white mb-0">
      <li class="nav-item">
        <a href="exam_stu.php" class="nav-link text-dark">
    ...
    <div class="page-content p-5" id="content">
        ...
        <hr>
        <!-- <a class="button" id="APBTN" href="#popup1"> <button type="button" class="btn btn-secondary" style="color:#9b59b6;width:900px;margin-left:1rem;margin-top:1rem;height:4rem;background:transparent;   border:1px solid #ccc;border-bottom-width: 3px;" onclick="myfunc()">Announce something to your class</button></a> -->
        <!-- <div class="col-lg-12">
            <div class="container">
        ...
        <div class="col-lg-12">
            <div class="container flex-2">
                <div class="pdf">
                    <h3>Instructions: </h3>
                    <h6 class="display-7 text-dark"><?php echo $e_details; ?></h6><br><br>
                    <embed type="application/pdf" src="<?php if (!empty($file)) echo $file; ?>" width="<?php if (!empty($file)) echo "600px";
                                                                                                        else echo "0px"; ?>" height="<?php if (!empty($file)) echo "600px";
                                                                                                                                        else echo "0px"; ?>"">
                </div>
                <div class=" vll">
                </div>
                <form method="post" enctype="multipart/form-data" style="margin-left:2rem;margin-top:4rem;">
                    <div class="inp">
                        <div class="boxx">
                            <h5><span style="color:#9b59b6;">Due Time: </span><?php echo $etime; ?></h5>
                            <h5><span style="color:#9b59b6;">Due Date: </span><?php echo $edate; ?></h5>
                            <h5> <span style="color:green;"><?php if (isset($fdate)) echo "Status: "; ?></span> <?php if (isset($fdate)) {
                                                                                                                    if ($fdate <= $edate) {
                                                                                                                        if ($ftime <= $etime)
                                                                                                                            echo "Turned in";
                        ...
                        <input type="submit" name="upload" class="btn btn-secondary" style="height:3rem;width:12rem;margin-top:1rem;"></input>
                    </div>
                </form>
            </div>
        </div>
